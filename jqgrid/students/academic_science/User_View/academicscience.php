...
$Model = array(
  ...
  array("name" => "name", "label" => "Subject", "autowidth" => true, "stype" => "select", "searchoptions" => array("value" => ":[All];Arts:Arts;Biology:Biology;Chemistry:Chemistry;English:English;Indonesian Language:Indonesian Language;Math:Math;Physics:Physics")),
...
$grid->setUrl('academicscience.php');
// Set some grid options
$grid->setGridOptions(array(
  "caption" => "SCIENCE MAJOR ACADEMIC PERFORMANCE",
  "rowNum" => 13,
...
$grid->setAutocomplete("names", "#id_student", "SELECT name, name, id_student FROM student WHERE name LIKE ? AND student.major='Science' ORDER BY name", null, true, false);

//$grid->setAutocomplete("name","#major","SELECT name, name, major FROM subject WHERE name LIKE ? ORDER BY name",null,true,false);

$grid->setAutocomplete("name", "#id_subject", "SELECT name, name, id_subject FROM subject WHERE name LIKE ? and major = 'Science' ORDER BY name", null, true, false);
